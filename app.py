...
def export_spc_pdf(spc, color):
    ...
    # COVER PAGE
    # =========================
    story.append(Paragraph("SPC Capability Report", styles["Title"]))
    story.append(Spacer(1, 12))
    story.append(Paragraph(f"<b>Color code:</b> {color}", styles["Normal"]))
    story.append(Paragraph("Process Capability & Distribution Analysis", styles["Normal"]))
    story.append(Spacer(1, 24))
    story.append(Paragraph("Generated by SPC Dashboard", styles["Italic"]))
    ...
    # =========================
    for k in spc:
        story.append(Paragraph(f"Factor: {k}", styles["Heading1"]))
        story.append(Spacer(1, 12))
        ...
        if values.std() > 0:
            x = np.linspace(values.mean() - 4 * values.std(),
                            values.mean() + 4 * values.std(), 400)
            pdf = normal_pdf(x, values.mean(), values.std())
            ax.plot(x, pdf * len(values) * (bins[1] - bins[0]), color="black")

        if lcl is not None:
            ax.axvline(lcl, color="red", linestyle="--")
        if ucl is not None:
            ax.axvline(ucl, color="red", linestyle="--")
        ...
        story.append(Paragraph(
            f"""
            <b>LINE Capability</b><br/>
            N = {len(values)}<br/>
            Mean = {values.mean():.3f}<br/>
            Std = {values.std():.3f}<br/>
            Ca = {ca}<br/>
            Cp = {cp}<br/>
            Cpk = {cpk}
            """,
